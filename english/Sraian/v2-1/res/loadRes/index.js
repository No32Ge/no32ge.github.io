 ...
 * 批量章节加载的进度回调。
 *
 * @callback ChapterProgressCallback
 * @param {number} done   - 当前已完成加载的章节数
 * @param {number} total  - 总章节数
 * @param {string} url    - 本次完成的章节 URL
 * @param {{ok: boolean, data?: any, error?: Error}} result 
 *   本次加载结果：成功 {ok:true,data}；失败 {ok:false,error}
 */
 ...
 */
const defaultProgressCallback = (done, total, url, result) => {
    console.log(`${done}/${total} loaded: ${url}`, result.ok ? 'OK' : result.error.message);
};

/**
 * @param {IndexInfo} index
 * @param {ChapterProgressCallback} [callback=defaultProgressCallback]
 */
async function loadIndex(index, callback = defaultProgressCallback) {
