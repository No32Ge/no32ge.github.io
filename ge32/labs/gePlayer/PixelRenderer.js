export class PixelRenderer {
  ...
  async fromImageFile(file, layerIndex=0, frameIndex=0) {
    ...
    for (let y = 0; y < height; y++) {
      ...
      for (let x = 0; x < width; x++) {
        const i = (y * width + x) * 4;
        layer[y][x] = { r: data[i], g: data[i+1], b: data[i+2], a: data[i+3] / 255 };
      }
    }
  ...
  exportGEPIC() {
    ...
    const blob = new Blob([header, meta, data], {type:'application/octet-stream'});
    ...
    for (let y = 0; y < this.height; y++) {
      for (let x = 0; x < this.width; x++) {
        ...
        for (let l = 0; l < layers.length; l++) {
          ...
          if (!px) continue;
          const alpha = px.a ?? 1;
          const pr = px.r * alpha, pg = px.g * alpha, pb = px.b * alpha;
          r = pr + r*(1-alpha);
          g = pg + g*(1-alpha);
          b = pb + b*(1-alpha);
          a = alpha + a*(1-alpha);
